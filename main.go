...
func main() {
	...
	newClient := Client{
		FIO:      "Салманов Магомед Салманович", // укажите ФИО
		Login:    "salmanovich95",               // укажите логин
		Birthday: "19940312",                    // укажите день рождения
		Email:    "salmanovich95@gmail.com",     // укажите почту
	}
	...
	newLogin := "salmanovich12" // укажите новый логин
...
func updateClientLogin(db *sql.DB, login string, id int64) error {
	_, err := db.Exec("UPDATE clients SET login = :login WHERE id = :id", sql.Named("login", login), sql.Named("id", id))
	if err != nil {
		return err
	}
	return nil
